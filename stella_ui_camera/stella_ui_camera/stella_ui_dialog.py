...
#		r.sleep()
	...
	def OnSlam(self, event):
		...
		if v == True:
			...
			self.robot_slam_proc = psutil.Popen("gnome-terminal --tab -- ros2 launch stella_cartographer cartographer.launch.py && sleep 2 && wmctrl -r 'stella_cartographer.rviz - RViz' -e 0,70,150,630,380",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
	...
	def OnNavigation(self, event):
		...
		if v == True:
			self.robot_nav_proc = psutil.Popen("gnome-terminal --tab -- ros2 launch stella_navigation2 navigation2.launch.py map:=" + self.path_navigation + "&& sleep 2 && wmctrl -r 'nav2_default_view.rviz - RViz' -e 0,70,150,630,380",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
		elif v == False:
